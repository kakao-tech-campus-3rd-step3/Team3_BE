...
public class PasswordOtpTokenTest {
    ...
    void token_Expired_After3Minutes() throws Exception {
        // given
        Field expiresAtField = otpToken.getClass().getSuperclass().getDeclaredField("expiryDate");
        expiresAtField.setAccessible(true);
        expiresAtField.set(otpToken, LocalDateTime.now().minusMinutes(3));

        // when & then
        assertThatThrownBy(otpToken::validateExpiryDate)
            .isInstanceOf(UnauthorizedException.class);
    }

    @Test
    @DisplayName("인증코드 발송 횟수 업데이트 - 하루가 지나면 인증번호 요청 횟수 자동 초기화된다")
    void resetTodayRequestLimit_NextDay_ResetsCount() throws NoSuchFieldException, IllegalAccessException {
